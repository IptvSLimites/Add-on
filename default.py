...
g_ignoreSetResolved=['plugin://plugin.video.f4mTester','plugin.video.dramasonline','plugin.video.shahidmbcnet','plugin.video.SportsDevil','plugin.stream.vaughnlive.tv','plugin.video.ZemTV-shani']
...
addon = xbmcaddon.Addon('plugin.video.iptv.sem.limites')
addon_base = 'live.streamspro'
...
token_conteudos = addon.getSetting('token_conteudos')
...
#exec('bGluayA9ICdodHRwczovL3Jhdy5naXRodWJ1c2VyY29udGVudC5jb20vSmFydmlzU3RyZWFtL1Bhc3N3b3JkL21hc3Rlci91c2VybmFtZT0lcyZwYXNzd29yZD0lcy50eHQnJSAodXNlcm5hbWUscGFzc3dvcmQp'.decode('base64'))
...
#exec(decode_base64_and_inflate(''))
link = 'https://raw.githubusercontent.com/IptvSLimites/Password/master/username=%s&password=%s.txt'% (username,password)

Link_Base = ''#<=== Colocar Link entre as ''
...
try:
	...
	link=response.read()
	response.close()
except urllib2.URLError, e:  # urllib2.HTTPError, e: COLOQUE SEU USUÁRIO E SENHA PARA ENTRAR NO ADDON
	xbmcgui.Dialog().ok(addon_base,'COLOQUE SEU USERNAME & PASSWORD PARA PODER TER ACESSO AO ADD-ON','')
	ADDON.openSettings()
	sys.exit(0)
else:
	pass		
...
def atalizacao():
	...
	if versao_zip_old1 != link:    	
		...
		req.add_header('User-Agent', 'Mozilla/5.0 (Windows; U; Windows NT 6.0; en-US; rv:1.9.0.6)')
		response = urllib2.urlopen(req)
		link = response.read()
		path = addonfolder
		...
		#f.close()
		response.close()
		UpdateMe_local_update()
			
def UpdateMe_local_update():
	xbmc.executebuiltin("UpdateLocalAddons")
	xbmc.executebuiltin("UpdateAddonRepos")      
	dialog=xbmcgui.Dialog(); dialog.ok( addon_base,  "ESTAMOS ATUALIZANDO O ADD-ON PRECIONE CONFIRMAR  E ENTRE NOVAMENTE",'')
	sys.exit(0)				

def getSources():
        atalizacao()
        try:
            addDir('[COLOR yellow][B]CONTEUDO ADULTO[/COLOR][/B]','',200,'https://i.imgur.com/HdUbLTF.png',FANART,'','','','')
            addDir('[COLOR yellow][B]ANIMES & DESENHOS[/COLOR][/B]','https://raw.githubusercontent.com/JarvisStream/Add-on/master/Home%20Animes%20%26%20Desenhos.xml',201,'https://i.imgur.com/BeL988m.png',FANART,'','','','')
            addDir('[COLOR yellow][B]DOCUMENTARIOS[/COLOR][/B]','https://raw.githubusercontent.com/JarvisStream/Add-on/master/Document%C3%A1rios.m3u',201,'https://i.imgur.com/sWTHTmA.png',FANART,'','','','')
            addDir('[COLOR yellow][B]FILMES[/COLOR][/B]','https://gist.githubusercontent.com/mvinfo/aeb35b7a6f2c8e261e023f8123b61fc4/raw/a08c3ae1b40b0adc832d9a4d83d5a4e5e0d56f9f/filmes.m3u',201,'https://i.imgur.com/Wjddv1h.png',FANART,'','','','')
			addDir('[COLOR yellow][B]RADIOS ONLINE[/COLOR][/B]','https://gist.githubusercontent.com/ctroncoso/0e804c6e8cf3953e11710a62412007a5/raw/d16cf139f09027e0223c8b53e35b3e62478308aa/playlist.m3u8',201,'https://i.imgur.com/gBhd0kv.png',FANART,'','','','')
            addDir('[COLOR yellow][B]SERIADOS[/COLOR][/B]','https://gist.githubusercontent.com/mvinfo/83f6732f88b9e2052d6aaa6065fee543/raw/fb19a5bd12728c70d01f55fc3e85729c9e1ecfeb/SeriesP-Z.m3u',201,'https://i.imgur.com/4g31XOo.png',FANART,'','','','')
            #addDir('[COLOR yellow][B]CATEGORIAS AQUI[/COLOR][/B]','LINKS AQUI',201,'LINKS LOGO AQUI',FANART,'','','','')
			#addDir('[COLOR yellow][B]CATEGORIAS AQUI[/COLOR][/B]','LINKS AQUI',201,'LINKS LOGO AQUI',FANART,'','','','')
            #addDir('[COLOR yellow][B]CATEGORIAS AQUI[/COLOR][/B]','LINKS AQUI',201,'LINKS LOGO AQUI',FANART,'','','','')
            #addDir('[COLOR yellow][B]CATEGORIAS AQUI[/COLOR][/B]','LINKS AQUI',201,'LINKS LOGO AQUI',FANART,'','','','')
			#addDir('[COLOR yellow][B]CATEGORIAS AQUI[/COLOR][/B]','LINKS AQUI',201,'LINKS LOGO AQUI',FANART,'','','','')
...
def getData(url,fanart, data=None):
    ...
    #print 'xxxxxxxxxxsoup',soup
        if len(soup('channels')) > 0:# and addon.getSetting('donotshowbychannels') == 'true':
...
#                print channel
                ...
                try:
                    ...
                    pass#addon_log('There was a problem adding directory from getData(): '+name.encode('utf-8', 'ignore'))
        else:
...
#                print 'setting as folder in link'
            ...
            if addon.getSetting('dlaudioonly') == 'true':
                contextMenu.append(('!!Download [COLOR seablue]Audio!![/COLOR]','XBMC.RunPlugin(%s?url=%s&mode=24&name=%s)'
                                        %(sys.argv[0], urllib.quote_plus(url), urllib.quote_plus(name))))
        elif url.endswith('.m3u8') or url.endswith('.ts'):#123
			if addon.getSetting('player_f4m') == "ATIVAR": ## PLAYER_TORRENT  url = 'plugin://plugin.video.f4mTester/?streamtype=HLSRETRY&amp;name='+name+'&amp;url='+url+'&amp;iconImage='+icon	
				try:
					if '.f4m' in url:
						...
						#url = 'plugin://plugin.video.f4mTester/?url='+urllib.quote_plus(url)+'&amp;streamtype=HLSRETRY&name='+name#HLSRETRY
					elif url.endswith('.ts'):
						mode = '12'
						url = 'plugin://plugin.video.f4mTester/?streamtype=TSDOWNLOADER&name='+name+'&url='+url+'&iconImage='+iconimage		
						#url = 'plugin://plugin.video.f4mTester/?url='+urllib.quote_plus(url)+'&amp;streamtype=TSDOWNLOADER&name='+name#TSDOWNLOADER	
				except: pass	
			elif addon.getSetting('player_f4m') == "DESATIVAR":
				mode = '12'
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False,regexs=None,reg_url=None,allinfo={}):
        ...
        if showcontext:
            contextMenu = []
            contextMenu.append(("Movie Information", "XBMC.Action(Info)","RunPlugin(%s?action=addView&content=movies)" % sys.argv[0]))
            parentalblock =addon.getSetting('parentalblocked')
            parentalblock= parentalblock=="true"
            parentalblockedpin =addon.getSetting('parentalblockedpin')
#            print 'parentalblockedpin',parentalblockedpin
...
def player_Torrents(url,name,iconimage):# Quasar|Pulsar|Yatp|Torrenter|XbmcTorrent|Torrentin|Ace Stream (apk)|Rox Player (magnet.*?)
	...
	if selfAddon.getSetting('player_torrent') == "Quasar":
		...
		if sistema == 'android':
			...
			sys.exit(0)		
	elif selfAddon.getSetting('player_torrent') == "Rox Player":
		local = selfAddon.getSetting('local_roxplayer').replace('ROX Player.lnk','ROX Player/roxplayer.exe').replace('Desktop','AppData\Local').replace('','').replace('','')
		xbmc.log('###################################                  INICIANDO SISTEMA       '+local+'        ################################################################')
		item = subprocess.call([local,'%s' % url])
		sys.exit(0)
	pl=xbmc.PlayList(1)
	pl.clear()
...
if mode==None:
    ...
    if token_conteudos=='':
        xbmcgui.Dialog().ok(addon_base,'VOCE ESQUECEU DE DEFINIR A SENHA','')
        sys.exit(0)	
    else:
        pass	
    keyboard = xbmc.Keyboard('','coloque a senha para poder entrar na categoria')
    keyboard.doModal()
    if not (keyboard.isConfirmed() == False):
        newStr = keyboard.getText()
        if newStr == token_conteudos:	
            url = 'https://raw.githubusercontent.com/JarvisStream/Add-on/master/Conteudo_Adulto.m3u' #Link Adulto Aqui
            getData(url,'', data=None)
            xbmcplugin.endOfDirectory(int(sys.argv[1]))
        else:
            xbmcgui.Dialog().notification(addon_base,'Opps vc errou a senha',icon)
            sys.exit(0)	
...
####################################                  INICIANDO SISTEMA       C:\Users\crotti\AppData\Local\ROX Player/roxplayer.exe        ################################################################
#
#C:\Users\crotti\AppData\Local\ROX Player
